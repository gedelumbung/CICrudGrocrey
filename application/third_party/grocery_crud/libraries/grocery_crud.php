...
class grocery_Layout extends grocery_Model_Driver
{
	...
	{	
		$ci = &get_instance();
		$ci->load->css('public/grocery_crud/css/ui/simple/jquery-ui-1.8.10.custom.css');		
		$ci->load->css('public/grocery_crud/css/jquery_plugins/ui.multiselect.css');
		$ci->load->js('public/grocery_crud/js/jquery_plugins/jquery-ui-1.8.10.custom.min.js');	
		$ci->load->js('public/grocery_crud/js/jquery_plugins/ui.multiselect.js');
		$ci->load->js('public/grocery_crud/js/jquery_plugins/config/jquery.multiselect.js');
	...
	{
		$ci = &get_instance();
		$ci->load->css('public/grocery_crud/css/other/fileuploader/fileuploader.css');
		$ci->load->js('public/grocery_crud/js/other/fileuploader.js');
		$ci->load->js('public/grocery_crud/js/other/fileuploader.config.js');
...
class grocery_States extends grocery_Layout
{
	...
	{ 
		...
		elseif($first_segment == 'upload_file')
		{
			$state_code = 11;
		}
		elseif($first_segment == 'delete_file')
		{
			$state_code = 12;
		}							
		else 
	...
	protected function getUploadUrl($field_name)
	{
		...
		return site_url($rsegements[1].'/'.$rsegements[2].'/upload_file/'.$field_name);
	}	
	...
	protected function getFileDeleteUrl($field_name)
	{
		...
		return site_url($rsegements[1].'/'.$rsegements[2].'/delete_file/'.$field_name);
	}	
...
class grocery_CRUD extends grocery_States
{
	 ...
	 */
	public function set_relation_1_n($field_name, $relation_table, $selection_table, $primary_key_alias_to_this_table, $primary_key_alias_to_selection_table , $title_field_selection_table , $priority_field_relation_table = null)
	{
